    ...
    try:
        ...
        if result.returncode != 0 and result.stdout:
            for line in result.stdout.strip().split("\n"):
                ...
                match = re.search(r"(\S+)\s+(\S+)\s+has requirement\s+(.+?),\s+but you have\s+(.+)", line)
                if match:
                    results.append(
                        DiagnosticResult(
                            category=category,
...
def check_import_source(project_root: Path, category: str) -> list["DiagnosticResult"]:
    ...
    try:
        installed = {
            pkg.metadata["Name"].lower(): pkg for pkg in __import__("importlib.metadata").metadata.distributions()
        }
