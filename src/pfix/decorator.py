...
def _handle_decorator_exception(
    fn, exc, attempt, max_retries, hint, args, kwargs, config, should_auto, should_restart, on_error
) -> bool:
    ...
    hints["args_types"] = str({f"arg{i}": type(a).__name__ for i, a in enumerate(args)})

    ctx = analyze_exception(exc, func=fn, hints=hints)
    console.print(f"[blue]🔍 Analyzing ({classify_error(ctx)})...[/]")
...
def _apply_decorator_fix(ctx, proposal, config, should_auto) -> bool:
    """Apply fix with temporary config override. CC≤3."""
    old_auto = config.auto_apply
    if should_auto:
        config.auto_apply = True
    try:
        return apply_fix(ctx, proposal, confirm=not should_auto)
    finally:
        config.auto_apply = old_auto


def _after_fix_action(fn, should_restart) -> bool:
    """Perform post-fix action (restart or reload). CC≤3."""
    if should_restart:
