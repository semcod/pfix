...
if __name__ == "__main__":
    tests = [
        ("1. Error chain (config → DB → API)", lambda: start_application()),
        (
        ...
        ("3. Transient failure (retry-worthy)", lambda: fetch_with_transient_failure()),
        ("4. Exception in cleanup (finally)", lambda: process_with_cleanup()),
    ]
